  ...
  <div v-if="open" class="modal-backdrop" @click.self="onCancel">
    <div class="modal-content" role="dialog" aria-modal="true">
      ...
      <div class="modalFooter">
        ...
        <BaseButton :disabled="confirmDisabled || loading" variant="primary" @click="onConfirm">
          <span v-if="loading">생성 중...</span>
          <span v-else>생성 및 분석 실행</span>
        </BaseButton>
      </div>
    </div>
  </div>
...
<style scoped>
.modalHeader h3{ margin:0 0 6px 0 }
...
.error{ margin-top:8px; color:var(--danger); font-weight:700 }
...
.modal-content{ background:#fff; padding:18px; border-radius:12px; width:520px; max-width:94%; box-shadow:0 20px 50px rgba(16,24,40,0.2) }
</style>
