...
export const useAuthStore = defineStore("auth", {
  ...
  actions: {
    setAccessToken(token) {
      ...
      // 디버그: 토큰 설정 시 로그 출력
      try {
        console.debug("[auth] setAccessToken", {
          token,
      ...
      try {
        ...
        if (!token && !this.user) {
          try {
            ...
            try {
              console.debug(
                "[auth] login: populated user via /me after signin"
              );
            } catch (e) {}
          } catch (e) {
            // /me 실패하면 로그인은 성공적이지만 토큰/유저 정보는 없음
            try {
              console.debug("[auth] login: /me after signin failed");
            } catch (e) {}
          }
        }
        return data;
    ...
    async refresh() {
      ...
      this._refreshPromise = (async () => {
        try {
          const res = await axios.post("/api/user/refresh", null, {
            withCredentials: true,
          });
          const data = res.data || {};
    ...
    async loginWithKakaoCode({ code, redirectUri }) {
      // 쿠키/세션 기반(RefreshToken 등)까지 고려해 withCredentials 필수
      ...
      // 토큰/유저가 없으면 쿠키 기반일 수 있으니 /me로 보강(조용히 시도)
      if (!token && !this.user) {
        try {
          ...
          // /me 실패해도 콜백 단계에선 일단 반환(다음에 checkAuth로 잡힘)
        }
      }
    ...
    async checkAuth() {
      ...
      try {
        try {
          console.debug(
            "[auth] checkAuth start, accessToken:",
            this.accessToken
          );
        } catch (e) {}
      } catch (e) {}
      try {
        ...
        if (this.accessToken) {
          try {
            ...
            try {
              console.debug(
                "[auth] checkAuth success (with existing token)",
                this.user
              );
            } catch (e) {}
            return true;
            ...
            if (status === 401) {
              ...
              try {
                console.debug(
                  "[auth] checkAuth success (after refresh)",
                  this.user
                );
              } catch (e) {}
              return true;
            ...
            this.isAuthenticated = false;
            try {
              console.debug("[auth] checkAuth failed, invalid token");
            } catch (e) {}
            return false;
          }
        }
        ...
        try {
          try {
            console.debug("[auth] no accessToken, attempting refresh");
          } catch (e) {}
          await this.refresh();
          const res = await axios.get("/api/user/me", {
            headers: { Authorization: `Bearer ${this.accessToken}` },
          ...
          try {
            console.debug("[auth] refresh succeeded, user:", this.user);
          } catch (e) {}
          return true;
        } catch (err) {
